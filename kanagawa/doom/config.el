...
(after! ox-latex
  (add-to-list 'org-latex-packages-alist '("" "kanagawadark"))
  (add-to-list 'org-latex-classes
               '("kanagawa-dark-article"
                 "\\documentclass[11pt]{article}
\\usepackage{kanagawadark}"
                 ("\\section{%s}" . "\\section*{%s}")
                 ("\\subsection{%s}" . "\\subsection*{%s}")
                 ("\\subsubsection{%s}" . "\\subsubsection*{%s}")
                 ("\\paragraph{%s}" . "\\paragraph*{%s}")
                 ("\\subparagraph{%s}" . "\\subparagraph*{%s}")))
  (setq org-latex-default-class "kanagawa-dark-article"))
...
(after! org
  (setq org-html-head
        (concat
         "<style type=\"text/css\">\n"
         (with-temp-buffer
           (insert-file-contents (expand-file-name "~/.config/doom/kanagawa-dark.css"))
           (buffer-string))
         "\n</style>")))
