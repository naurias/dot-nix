...
(setq doom-theme 'doom-kanagawa-dragon)
(after! doom-themes
...
(after! ox-latex
  (add-to-list 'org-latex-packages-alist '("" "kanagawadragon"))
  (add-to-list 'org-latex-classes
               '("kanagawa-dragon-article"
                 "\\documentclass[11pt]{article}
\\usepackage{kanagawadragon}"
                 ("\\section{%s}" . "\\section*{%s}")
                 ("\\subsection{%s}" . "\\subsection*{%s}")
                 ("\\subsubsection{%s}" . "\\subsubsection*{%s}")
                 ("\\paragraph{%s}" . "\\paragraph*{%s}")
                 ("\\subparagraph{%s}" . "\\subparagraph*{%s}")))
  (setq org-latex-default-class "kanagawa-dragon-article"))
...
(after! org
  (setq org-html-head
        (concat
         "<style type=\"text/css\">\n"
         (with-temp-buffer
           (insert-file-contents (expand-file-name "~/.config/doom/kanagawa-dragon.css"))
           (buffer-string))
         "\n</style>")))
