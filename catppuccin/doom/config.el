...
(setq doom-theme 'doom-catppuccin-mocha)
(after! doom-themes
...
(after! ox-latex
  (add-to-list 'org-latex-packages-alist '("" "catpuccinmocha"))
  (add-to-list 'org-latex-classes
               '("catpuccinmocha-article"
                 "\\documentclass[11pt]{article}
\\usepackage{catppuccinmocha}"
                 ("\\section{%s}" . "\\section*{%s}")
                 ("\\subsection{%s}" . "\\subsection*{%s}")
                 ("\\subsubsection{%s}" . "\\subsubsection*{%s}")
                 ("\\paragraph{%s}" . "\\paragraph*{%s}")
                 ("\\subparagraph{%s}" . "\\subparagraph*{%s}")))
  (setq org-latex-default-class "catpuccinmocha-article"))
...
(after! org
  (setq org-html-head
        (concat
         "<style type=\"text/css\">\n"
         (with-temp-buffer
           (insert-file-contents (expand-file-name "~/.config/doom/catpuccin-mocha.css"))
           (buffer-string))
         "\n</style>")))
